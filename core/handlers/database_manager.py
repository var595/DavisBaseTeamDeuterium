...
from core.decoders.sql_command_handler import sql_command_handler
from core.decoders.output_format import output_format

from core.decoders.sql_query_parser import sql_query_parser
...
class database_manager:
    ...
    def command_parse(command: str, tables: Dict, indexes: Dict):
        if command.lower() == "exit;":
            ...
            # try:
                ...
                sql_command_handler.router(parsed_tokens, tables, indexes)
                database_manager.save_to_disk(tables, indexes)
    ...
    def create_database(tables: Dict, indexes: Dict):
        sql_command_handler.router(create_db.create_system_tables(), tables, indexes)
        sql_command_handler.router(create_db.create_system_columns(), tables, indexes)
        
        for entry in create_db.fill_system_tables():
            try:
                sql_command_handler.router(entry, tables, indexes)
            except:
                traceback.print_exc()
    ...
    def drop_database(tables,indexes):
        ...
        if os.path.exists(os.path.join(exec_path,config_manager.get_data_dir())):
            tables.clear()
            indexes.clear()
            for root, dirs, files in os.walk(os.path.join(exec_path,config_manager.get_data_dir())):
                for f in files:
    ...
    def save_to_disk(tables,indexes):
        ...
        for k, tab in tables.items():
            if k not in {"system_tables", "system_columns"}:
                ...
                sql_command_handler.router(parsed_tokens, tables, indexes)
                file_path = os.path.join(exec_path,config_manager.get_data_dir(),f"{k}{tbl_ext}")
        ...
        for k,ndx in indexes.items():
            file_path = os.path.join(exec_path,config_manager.get_data_dir(),f"{k}{ndx_ext}")
            with open(file_path, 'wb') as f:
    ...
    def load_db(tables: Dict, indexes: Dict):
        ...
        if os.path.exists(system_tables) and os.path.exists(system_columns):
            with open(system_tables, 'rb') as f:
                output_format.disable_stdout()
                table = Table.bytes_to_object(f.read(), config_manager.get_page_size(),2, create_db.system_tables_column_data(), "system_tables")
                output_format.enable_stdout()
                tables["system_tables"] = table
                f.close()

            with open(system_columns, 'rb') as f:
                output_format.disable_stdout()
                table = Table.bytes_to_object(f.read(), config_manager.get_page_size(), 11, create_db.system_columns_column_data(), "system_columns")
                output_format.enable_stdout()
                tables["system_columns"] = table
                f.close()

            pdict = sql_query_parser("select id_row from system_tables where table_name = 'system_tables';").parse()[0]
            pdict["ret_mode"] = True
            table_rec_count = sql_command_handler.router(pdict, tables, indexes)
            
            pdict = sql_query_parser("select id_row from system_tables where table_name = 'system_columns';").parse()[0]
            pdict["ret_mode"] = True
            col_rec_count = sql_command_handler.router(pdict, tables, indexes)
            
            tables["system_tables"].id_row = table_rec_count[-1][-1]
            tables["system_columns"].id_row = col_rec_count[-1][-1]

            pdict = sql_query_parser("show tables;").parse()[0]
            pdict["ret_mode"] = True
            table_names = sql_command_handler.router(pdict, tables, indexes)
            for t in table_names:
                    table_name = t[-1]
                    if table_name not in tables:
                        table_name = table_name.lower()
                        table_path = os.path.join(exec_path,config_manager.get_data_dir(),f"{table_name}{tbl_ext}")
            
                        cdata = system_table.parse_column_data(table_name)
                        cdata["ret_mode"] = True
                        cdata_list = sql_command_handler.router(cdata, tables, indexes)            
                        ...
                        tdata["ret_mode"] = True
                        output = sql_command_handler.router(tdata, tables, indexes)
