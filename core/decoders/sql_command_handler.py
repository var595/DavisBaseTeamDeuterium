...
class SQLCommandHandler:
    ...
    @classmethod
    def router(cls, parsed_tokens, in_memory_tables, in_memory_indexes: Dict):
    ...
    @classmethod
    def get_table(
        cls, tname: str, in_mem_tables: Dict, in_mem_idx: Dict, creation_mode=False
    ):
        tname = tname.lower()
    ...
    def create_table(cls, table_name: str, column_list: List[Dict], cache: Dict):
        ...
        cache_tables, cache_indexes = map(
            lambda x: cache[x], ["cache_tables", "cache_indexes"]
        )
        ...
        if not cls.get_table(
            ...
            if table_name not in {"system_tables", "system_columns"}:
                ...
                col_rec_count = SQLCommandHandler.router(
                    parsed_tokens, cache_tables, cache_indexes
                )

                parsed_tokens = system_table.update_table_data(
                    "system_columns", col_rec_count[-1][-1] + len(column_list)
                )
                SQLCommandHandler.router(parsed_tokens, cache_tables, cache_indexes)
        ...
        if table_name not in {"system_tables", "system_columns"}:
            ...
            pdict = system_table.insert_table_data(
                table_rec_count + 1, table_name, config_manager.get_page_size(), 0
            )
    ...
    def drop_index(cls, table_name: str, column_name: str, cache: Dict):
        ...
        if name in cache_indexes:
            file_path = os.path.join(
                exec_path, config_manager.get_data_dir(), f"{name}{ndx_ext}"
            )
            os.remove(file_path)
    ...
    def drop_table(cls, table_name: str, cache: Dict):
        ...
        if table_obj := cls.get_table(table_name, cache_tables, cache_indexes):
            file_path = os.path.join(
                exec_path, config_manager.get_data_dir(), f"{table_name}{tbl_ext}"
            )
            os.remove(file_path)
