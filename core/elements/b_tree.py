...
class b_tree:
    ...
    def search(self, node: b_tree_node, key: Union[data_pointer, int]) -> Tuple[b_tree_node, int]:
        i = 0
        n = len(node.keys)
    ...
    def insert(self, key: Union[data_pointer, int]):
        current_root = self.root

        if len(current_root.keys) == self.max_ptr_degree() - 1:
            new_node = b_tree_node(False)
            self.root = new_node
    ...
    def split_child(self, node: b_tree_node, ptr_idx: int):
        
        node_to_split = node.pointers[ptr_idx]
        new_node = b_tree_node(node_to_split.is_leaf)
    ...
    def insert_available(self, node: b_tree_node, key: Union[data_pointer, int]):
        i = 0
        n = len(node.keys)
    ...
    def delete(self, node: b_tree_node, key: Union[data_pointer, int]):
    ...
    @staticmethod
    def merge(left: b_tree_node, median_key: Union[data_pointer, int], right: b_tree_node):
        left.keys = left.keys + [median_key] + right.keys
        left.pointers.extend(right.pointers)

    def check_node_fill(self, node: b_tree_node):
        return len(node.keys) >= self.min_ptr_degree()


    @staticmethod
    def find_predecessor(node: b_tree_node):
        if not node.is_leaf:
            return b_tree.find_predecessor(node.pointers[-1])
        else:
            return node.keys[-1]

    @staticmethod
    def find_successor(node: b_tree_node):
        if not node.is_leaf:
            return b_tree.find_successor(node.pointers[0])
        else:
            return node.keys[0]
